...
`tools/list` at `https://gateway.pipeworx.io/celestrak/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_satellite \
  -H 'Content-Type: application/json' \
  -d '{"norad_id":25544}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_satellite`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
